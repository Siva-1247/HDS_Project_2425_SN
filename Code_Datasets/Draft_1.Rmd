...
Complementing these behavioural insights, recent studies have begun to quantify spatial inequities in vaccination service availability. A UK-based analysis using Gini coefficients revealed that rural areas consistently experienced poorer geographic access to vaccine centres compared to urban counterparts [@CHEN2024103881].
Barriers to Vaccination: Ireland’s COVID-19 vaccination campaign was supported by a diversified service delivery network designed to maximise accessibility, equity, and speed of distribution. This network was composed of three primary modes of vaccine administration: mass vaccination centres, general practitioners (GPs), and community pharmacies. The configuration and expansion of these services were shaped by evolving public health priorities, population needs, and logistical feasibility throughout 2021. Three distinct service delivery mechanisms formed the operational backbone of Ireland’s COVID-19 vaccination rollout and were used to develop spatial variables representing healthcare access and vaccination infrastructure:
...
Despite Ireland achieving over 90% uptake of at least one COVID-19 vaccine dose among adults by the end of 2021, as reported by the Health Protection Surveillance Centre (HPSC) [@hpsc2021reports; @hpsc2021week52], spatial inconsistencies emerged at finer geographic scales. Disaggregated data at the Local Electoral Area (LEA) level revealed notable disparities, particularly in communities marked by higher socio-economic deprivation and limited healthcare access. These sub-national variations in vaccine uptake remain underexamined in the Irish research landscape and have not yet been systematically modelled using integrated spatial, demographic, and temporal data sources.
In contrast, spatial epidemiological approaches have proven effective in other jurisdictions. For instance, de Figueiredo (2021) applied multilevel regression and post-stratification techniques to map vaccine hesitancy across the UK, identifying clusters of low intent that correlated strongly with ethnicity, economic precarity, and urban residency [@nature2022mrp]. Similarly, McGowan et al. (2022) demonstrated that even in high-income settings with wide vaccine availability, uptake remained uneven—often reflecting underlying inequalities tied to deprivation levels [@mcgowan2022covid]. These findings suggest that areas with persistent deprivation and low vaccine uptake may face disproportionately greater risks in future outbreaks, reinforcing the value of sub-national, place-based public health interventions.
Research Gap:  the Irish context, this represents a significant research gap. Ireland’s unique settlement patterns—combining densely populated urban centres (notably the Greater Dublin Area) with widely dispersed rural and remote communities—create spatial dynamics that differ substantially from those documented in other countries. The use of spatiao-temporal models to understand COVID-19 vaccination uptake remains limited. In particular, the integration of demographic data, healthcare accessibility metrics, and vaccination coverage over time has not been fully explored.
In response, this thesis undertakes a spatial and demographic analysis of COVID-19 vaccination uptake for people 12 years and above in Ireland. Using LEAs as the unit of observation, it integrates official vaccination data, CSO demographic indicators, and engineered measures of healthcare accessibility to examine whether spatial inequities exist, identify their key correlates, and inform geographically targeted public health strategies.
*Introduction*
The COVID-19 pandemic placed extraordinary demands on global health systems and prompted one of the fastest vaccine development and distribution efforts in history. As vaccination campaigns scaled up, they brought with them a new public health challenge: ensuring that vaccines reached populations equitably. Although Ireland’s national campaign achieved high overall coverage the story beneath the national averages reveals more complexity[@hpsc2021week52]
. Evidence from Ireland and abroad suggests that uptake has varied significantly across regions and population subgroups, shaped by underlying social, economic, and geographic inequalities.

Understanding why vaccine uptake differs between communities is essential for building more resilient and equitable public health systems. Research shows that factors such as deprivation, education, age structure, and geographic access influence whether individuals get vaccinated—not just personal belief or awareness [@bmj_vaccine_hesitancy_2021] [@bmc_spatial_vax_2023]. Yet much of the current literature on vaccine equity focuses either on individual attitudes or national-level trends, with fewer studies examining how spatial and structural variables intersect at local levels to influence coverage. In Ireland, where official data are published at the LEA level, there is a valuable but underused opportunity to explore how demographic and geographic conditions shape vaccination outcomes in smaller, more granular population units.

This thesis addresses the under explored intersection of spatial access, demographic structure, and vaccination uptake in Ireland. Specifically, it investigates how demographic characteristics (e.g., age, education, general health outcomes) and spatial accessibility to vaccine services shaped COVID-19 vaccine coverage at the LEA level. This study applies both exploratory and statistical techniques to examine how structural variables—such as education, gendered unemployment, age dependency, and proximity to vaccination centres—are associated with local coverage rates. By applying a spatial-temporal lens to model vaccine uptake, this work contributes to the growing field of place-based public health research and offers empirical insights to support more equitable immunisation strategies in Ireland and beyond.
...
•	Data on the locations of mass vaccination centres (35 in total), GP clinics (1557), and community pharmacies (1041 in total) were obtained from the official HSE website. These represented the full network of vaccine delivery sites active during the 2021 national rollout.
Vaccination Uptake
•	Vaccine uptake rates were sourced from the CSO, specifically Table CDC47, which provides the proportion of the population vaccinated in the primary vaccine dose, booster 1 and booster 2 at the LEA level monthly. This dataset is the most granular, consistent public source of COVID-19 vaccine coverage in Ireland.
...
Age Composition: Extracted from SAP2022T1T1LEA22, individual-level age data were to be subsequently aggregated into five analytically relevant age groups (12–17, 18–54, 55–64, 65–70, and 71+ years). These groups were not selected arbitrarily, but to reflect the sequenced eligibility criteria of Ireland’s COVID-19 vaccination rollout, which was largely structured by age and risk )[@gov_vax_strategy_2020].:
...
Self-Reported Health Status: From SAP2022T12T3LEA22, respondents classified their general health using a 5-point Likert scale:
o	Very Good
o	Good
o	Fair
o	Bad
o	Very Bad
These categories capture perceived physical and mental health, which may influence vaccine-seeking behaviour.
Educational Attainment: This was drawn from SAP2022T10T4LEA22, where the thirteen original response categories were to be grouped into the six analytically meaningful levels: no formal education, primary education, upper secondary, apprenticeship/trade, honours bachelor degree, and postgraduate qualifications.
These reflect major stages in the Irish education system and allow analysis of educational gradients in vaccine uptake.
...
LEA boundary shapefiles were obtained from Ordnance Survey Ireland (OSI) via the CSO GeoHive Portal. They provided the official digital geographic boundaries necessary for spatial analysis and mapping applications. These boundary files serve as the foundational geographic framework for linking vaccination rates with corresponding service locations.
3.2 Data Collection Methods
Each dataset underwent systematic extraction, cleaning, and integration using a combination of R, Python, and geospatial tools. The following describes how data were collected and prepared for analysis.
...
Deprivation and Unemployment Indicators
The 2022 Pobal HP Index was obtained by directly contacting a liaison working at Pobal and the data shared with us was downloaded in a CSV format. 
...
The vaccination uptake data with the LEA geometries remained separate and were merged with the final analytical dataset based on the research question like the vaccination dose of interest that was studies etc.
...
All cross-sectional models used Beta regression with a logit link function, which is ideal for modelling proportional data as it ensures predictions remain in (0, 1). While the Beta distribution allows for heteroskedasticity through its precision parameter (phi), in our models, phi was assumed to be constant across all observations and was not modelled as a function of predictor variables.
...
The spatial correlation parameter (car) was estimated at 0.56 (95% CI: 0.03 to 0.99), suggesting moderate positive spatial autocorrelation between neighboring LEAs, i.e.,  56% of unexplained variance was attributable to spatial autocorrelation, indicating strong neighborhood-level similarities.
